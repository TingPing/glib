...
/*
 * This can work but:
 * - It usually returns NULL for bundle url
 * - Mixing mac icons and gtk icons is odd
 */
#if 0
GIcon *
g_content_type_get_icon (const gchar *type)
{
  ...
  CFStringRef uti, icon_name;
  CFURLRef bundle_url;
  gchar *icon_file, *cicon_name, *cbundle_url;
  GFile *file;
  GIcon *icon;
  ...
  dict = UTTypeCopyDeclaration (uti);
  ...
  if (!(icon_name = (CFStringRef)CFDictionaryGetValue (dict, kUTTypeIconFileKey)))
  {
    CFRelease (uti);
    return NULL;
  }

  bundle_url = UTTypeCopyDeclaringBundleURL (uti);
  if (!bundle_url)
  {
    g_message ("no url..");
    CFRelease (uti);
  ...
  cbundle_url = create_cstr_from_cfstring (CFURLGetString (bundle_url));
  cicon_name = create_cstr_from_cfstring (CFStringCreateCopy (NULL, icon_name));
  CFRelease (bundle_url);

  icon_file = g_build_filename (cbundle_url, "Contents", "Resources", cicon_name, NULL);
  g_free (cbundle_url);
  g_free (cicon_name);
