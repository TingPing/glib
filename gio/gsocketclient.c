...
static void connection_attempt_free (gpointer attempt);
...
g_socket_client_async_connect_data_free (GSocketClientAsyncConnectData *data)
{
  ...
  g_slist_free_full (data->connection_attempts, connection_attempt_free);
...
static void
connection_attempt_free (gpointer data)
{
...
connection_attempt_remove (ConnectionAttempt *attempt)
{
  ...
  connection_attempt_free (attempt);
}
...
g_socket_client_async_connect_complete (ConnectionAttempt *attempt)
{
  ...
  g_debug("g_socket_client_async_connect_complete()");
  ...
  /* Stop all other attempts */
  connection = g_steal_pointer (&attempt->connection);
  g_slist_free_full (data->connection_attempts, connection_attempt_free);
  data->connection_attempts = NULL;

  g_task_return_pointer (data->task, connection, g_object_unref);
				    ...
				    gpointer      user_data)
{
  ...
  g_debug("g_socket_client_connected_callback()");
...
on_connection_attempt_timeout (gpointer data)
{
  ...
  g_debug("on_connection_attempt_timeout()");
				     ...
				     gpointer      user_data)
{
  ...
  g_debug("g_socket_client_enumerator_callback()");
  ...
  if (address == NULL)
    {
      g_debug("g_socket_client_enumerator_callback() - address = NULL");
  ...
  attempt = g_new (ConnectionAttempt, 1);
  attempt->data = data;
  ...
  g_debug("g_socket_client_enumerator_callback() - post-connecting");
}
			       ...
			       gpointer             user_data)
{
  ...
  g_debug ("g_socket_client_connect_async()");
