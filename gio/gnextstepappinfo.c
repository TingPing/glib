...
static gboolean
g_nextstep_app_info_equal (GAppInfo *appinfo1, GAppInfo *appinfo2)
{
 ...
 */
static gchar *
get_bundle_string_value (NSBundle *bundle, NSString *key)
{
  NSString *value;
  const gchar *cvalue;
  gchar *ret;
                      ...
                      gboolean is_file)
{
  ...
  if (!url)
    g_debug ("Creating CFURL from %s %s failed!\n", cstr, is_file ? "file" : "uri");
                            ...
                            gboolean are_files)
{
  ...
  for (lst = uris; lst != NULL && lst->data; lst = g_list_next(lst))
  {
...
static gboolean
g_nextstep_app_info_launch_uris (GAppInfo *appinfo, GList *uris, GAppLaunchContext *launch_context,
                GError **error)
{
...
g_app_info_get_all_for_type (const char *content_type)
{
  ...
  /* The api should have this but instead they use non-portable
  * mimetypes instead.. so lets work around that */
                                 ...
                                 gboolean must_support_uris)
{
  ...
  if (!bundle_id)
  {
    g_warning("No default handler found for mimetype '%s'.\n", content_type);
    return NULL;
  }
...
g_app_info_get_default_for_uri_scheme (const char *uri_scheme)
{
  ...
  if (!bundle_id)
  {
    g_warning("No default handler found for url scheme '%s'.\n", uri_scheme);
    return NULL;
  }
